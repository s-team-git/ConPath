...
The next design milestone is to validate external-method interfaces and dataset provenance, including a discrepancy between the FlatLands paper's nominal spatial resolution and the local release metadata, before expanding physical-footprint claims or scheduling expensive runs. The clean three-seed no-event/no-global matrix remains a required causal experiment under its frozen contract; the three stopped no-event runs are now undergoing resumed exact validation, with no-global training queued. Before another UnScenes3D matrix, a training-only observation model audit must address the bound without held-out test labels. Scalable training-operator evidence, the external-method comparison, and a frozen test protocol remain necessary. This remains a working draft; final public-data, transfer, and submission-ready claims are unsupported.

The clean no-event/no-global training matrix was launched on 2026-09-07 under the frozen [ablation analysis plan](CLEAN_ABLATION_PLAN.md), paused with three no-event runs at completed epoch 4, and resumed on 2026-09-08 at 01:42 UTC from those complete-epoch checkpoints. At the user's request, execution was paused again at 03:47 UTC: the three no-event runs had completed 12/9/17 epochs and reached patience=8, while their final exact validation remained incomplete; no-global had not started. All processes exited. Training configuration, selected checkpoints, optimizer state and random states are preserved. At 03:59 UTC, a new explicit user request resumed the matrix: exact validation restarted from the selected weights and saved RNG state, without an additional training epoch; queued no-global training follows automatically after run audits. No ablation result is claimed before all six seeds/runs complete their audits. The approved prospective experimental deliverables are recorded in [EXPERIMENT_DESIGN_ZH.md](EXPERIMENT_DESIGN_ZH.md); external comparisons are required before broad comparative claims.
