...
<section id="scores" class="section"><div class="section-heading"><h2>同条件比较结果</h2><span class="badge">小规模开发实验</span></div><p class="section-description">通路概率由实际二值世界的精确连通性投票得到；确定性方法只输出1张图。按父级地点平均，全部模型用同一查询及有效区域。下表不与旧队列或论文公开分数混排行。</p><div class="table-scroll"><table><thead><tr><th>方法</th><th>输出数</th><th>通路Brier ↓</th><th>30%覆盖误判 ↓</th><th>未知区域IoU ↑</th></tr></thead><tbody>{''.join(rows)}</tbody></table></div><p class="caption">±为3个训练种子的标准差；IoU为采样平均，未选取最好的样本。半径先验只拟合训练事件，没有地图输出。</p><figure><a href="assets/zh/parent-pilot/baseline-comparison.svg"><img src="assets/zh/parent-pilot/baseline-comparison.svg" alt="七种方法的通路误差及固定覆盖率风险对照，含训练波动" style="display:block;width:100%;height:auto"></a><figcaption class="caption">绿色、棕色分别标出ConPath的误差、风险；灰色为对照。<a href="assets/zh/parent-pilot/baseline-comparison.pdf">下载PDF</a></figcaption></figure></section>
