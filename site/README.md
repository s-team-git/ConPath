...
independent-decoder, and deterministic mean-map controls over three seeds with equal-scene
weighting. The curves are validation-only diagnostics: the test split is not opened, and the
mean-map control is intentionally shown as a binary-risk trade-off.
