(() => {
  ...
  try {
    ...
    let index = 0, preferredModel = data.default_model || 'correlated', model = preferredModel, view = 'sample', source = 'all';
    ...
    function render() {
      ...
      model = modelOptions.some(option => option.value === preferredModel && !option.disabled) ? preferredModel : 'correlated';
      ...
      const modelName = {coherent_categorical:'ConPath改进版', correlated:'原ConPath', independent:'独立单元对照'}[model];
      ...
      $('[data-panel="prediction"] figcaption span').textContent = '② ' + modelName + '预测';
      ...
      const cohort = row.cohort === 'new_pilot' ? '本轮独立地点开发验证，非最终测试' : '旧模型开发诊断';
      const paired = row.panels.coherent_categorical ? `三种模型使用相同种子${row.seed}、输入与查询；验证反馈已用于改进设计，属于开发复用。` : '';
      const fallback = model !== preferredModel ? '此案例未提供所选模型，已显示原ConPath。' : '';
      $('#case-source').textContent = `${identifiers.join(' · ')} · ${cohort} · 当前：${modelName} · 机器人半径${row.radius_cells}格。${row.checkpoint_scope_zh}。` + paired + fallback + (row.selection === 'historical_label_selected' || row.samples === 128 ? `历史解释性案例：通路概率来自原${row.samples}次评估，示例图使用另一组固定随机流。` : `本图固定显示第1次采样，逐格概率和有路概率均来自同一批${row.samples}次采样。`);
    ...
    $('#home-model').addEventListener('change', event => { preferredModel = event.target.value; render(); });
