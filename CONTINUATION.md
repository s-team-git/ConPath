...
相对原模型的配对区间仍跨零，可通行类IoU略降，只支持开发改善趋势。详见[COHERENT_RESULTS_ZH.md](COHERENT_RESULTS_ZH.md)。

首页保持40个案例，前10个增加改进版/原模型/独立对照切换，共20张新面板；全部使用保存的真实输出。
完整两种子表与K4/K32图在`site/coherent.html`，原三种子基线在`site/pilot.html`。
本轮不新增训练、重新推理或原始归档读取。数值/图片/浏览器/线上版本凭据统一在
`results/coherent_parent_pilot_v1/publication/completion.json`；若已通过，不重复训练、评估或发布。

下一阶段先制定更充分的数据规模、共同收敛预算及强外部基线协议，同时将零查询事件权重修复纳入新版本，
原模型与改进版共同重训。最终留出须审核历史访问、排除已参与开发地点后再冻结；当前没有启动这一阶段。
两个任务、约60GiB项目显存预算的偏好继续有效。以下旧的“当前/下一项”按日期视为历史。



## 历史交付：模型效果首页重排（2026-09-09）
