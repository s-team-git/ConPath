...
> 本文保留历史运行记录；当前有效主表见 [PAPER_EVIDENCE.md](PAPER_EVIDENCE.md)。旧集成对照仍需在
> 修正支持范围后的协议下补齐，不能称为 PaSCo 的原版复现。UnScenes3D 均值地图当前以 v2 为准。
...
event. The checked official loader uses camera intrinsics for per-frame depth/normal processing but
does not expose an ego-pose/world-map stream, and the release's pair-level counts do not establish
our required sequence/site-held-out gate.
...
strict audits; event Brier is `0.51142` (correlated) versus `0.51130` (independent), using the final-support-projected v2 mean-map replay on the two
